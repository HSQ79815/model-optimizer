...
def onnx_res2_pth_res():
    ...
    allones_0 = np.zeros((1, 8400, 6), dtype=np.float32)
    allones_0[..., :2] = grids
    expanded_strides = np.concatenate(expanded_strides, 1)
    expanded_strides = expanded_strides.astype(np.float32)

    allones_1 = np.ones((1, 8400, 6), dtype=np.float32)
...
pad_8400_pads = onnx.helper.make_tensor(
    "pad_8400_pads", 7, [6], np.array([0, 0, 2, 0, 0, 2], dtype=np.int64))

# reshape_8400_shapes = onnx.helper.make_tensor(
#     "reshape_8400_shapes", 7, [2], np.array([8400, 6], dtype=np.int64))
...
slice_end_5 = onnx.helper.make_tensor(
    "slice_end_5", 6, [1], np.array([6], dtype=np.int32))
...
# reshape_8400_6 = onnx.helper.make_node(
#     'Reshape',
#     inputs=['add_8400_output', 'reshape_8400_shapes'],
#     outputs=['reshape_8400_6_output'],
#     name="add_8400"
# )
...
mul_8400_0 = onnx.helper.make_node(
    'Mul',
    inputs=['slice_8400_4_output', "slice_8400_5_output"],
...
concat_8400_0 = onnx.helper.make_node(
            'Concat',
            inputs=["sub_8400_0_output","sub_8400_1_output","add_8400_2_output","add_8400_3_output","mul_8400_0_output"],
            outputs=['concat_8400_0_output'],
            axis=2
        )

model.graph.output[0].type.tensor_type.shape.dim[2].dim_value=5
model.graph.output[0].name = "concat_8400_0_output"
