...
def covert2trt(model: onnx.ModelProto,profile_shapes ,fp16 ,engine_plan_path):
    logger = trt.Logger(trt.Logger.VERBOSE)
    builder = trt.Builder(logger)
    network = builder.create_network(
        (1 << int(trt.NetworkDefinitionCreationFlag.EXPLICIT_BATCH)) | (1 << int(trt.NetworkDefinitionCreationFlag.EXPLICIT_PRECISION)))
    ...
    for i in range(len(profile_shapes)):
        ...
        min_shape = profile[input_name][0]
        opt_shape = profile[input_name][1]
        max_shape = profile[input_name][2]
